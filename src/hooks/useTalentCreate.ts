...
export function useTalentCreate() {
  ...
  const handleChange = useCallback(
    (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
      ...
      const numValue =
        name === 'workingMode' || name === 'locationId' || name === 'currentPartnerId'
          ? Number(value) || undefined
          : value;
      updateBasicField(name as keyof typeof basicFormData, numValue);
    },
    [updateBasicField, basicFormData]
  );
  ...
  const handleSubmit = useCallback(
    async (e: React.FormEvent) => {
      ...
      if (basicFormData.workingMode === undefined || (basicFormData.workingMode as number) === 0) {
      ...
      try {
        ...
        const payload: TalentWithRelatedDataCreateModel = {
          ...
          initialCV: initialCVs.length > 0 && initialCVs[0].jobRoleLevelId && initialCVs[0].jobRoleLevelId > 0 ? {
            ...
            version: initialCVs[0].version || 1,
            cvFileUrl: initialCVs[0].cvFileUrl || '',
